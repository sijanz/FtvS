...
import SoFTlib.*;  import static SoFTlib.Helper.*;  import java.util.*;
...
{ int      schritt,    // Z�hlt die durchgef�hrten Arbeitsschritte.
  ...
  { while (schritt < Ftvs18_Aufg2.korrektEndSchritt)
    { String seInhalt  = schritt + " " + zustand,
             emInhalt  = null;
      int    emZustand = 10000;
      try { if      (sender()    == myChar())  sende(seInhalt, empfaenger());
  ...
  int schritt()  { return schritt; }

  int zustand()  { return zustand; }

  void setzeSchritt(int schritt)  { this.schritt = schritt; }

  void setzeZustand(int zustand)  { this.zustand = zustand; }
...
{ 
  ...
  { int schritt; Injektion nf;
	  
    Injektion(int schritt)  { this.schritt = schritt;  nf = null; }
  }
  ...
  { if (2 <= schritt && schritt <= 200)
      ...
      { anzFehler++;  return 1 + zufall.nextInt(291);
      }
...
{ static final int   korrektEndSchritt = 290;
  ...
  { boolean korrekt     = true;
    ...
    { minAnzFehler  = 999999;  maxAnzFehler  = 0;
      minAnzRL      = 999999;  maxAnzRL      = 0;
      minAnzZurueck = 999999;  maxAnzZurueck = 0;
      minAnzWeite   = 999999;  maxAnzWeite   = 0;
      minAnzAnfang  = 999999;  maxAnzAnfang  = 0;
    }
    ...
    if (korrekt) { if      ( ! mitZurueck)                 return 0;
